    ...
    for c in contours:  #Iterate through all contours.
        ...
        if contour_area_temp > contour_area_max:
            ...
            if contour_area_temp > 300:  # Only when the area is greater than 300, 
                area_max_contour = c #the contour with the largest area is considered valid, in order to filter out interference.
...
def move():
    ...
    # Placement coordinates (x, y, z) of different colored blocks.
    coordinate = {
    ...
    while True:
        if __isRunning:
            if first_move and start_pick_up: # When an object is detected for the first time.             
                ...
                setBuzzer(0.1)               
                result = AK.setPitchRangeMoving((world_X, world_Y - 2, 5), -90, -90, 0) # If the run time parameter is not filled in, the run time will adapt automatically.
                if result == False:
                    unreachable = True
                else:
                    unreachable = False
                ...
                first_move = False
                action_finish = True
            elif not first_move and not unreachable: # Not the first time an object is detected.
                set_rgb(detect_color)
                ...
                if start_pick_up: #
                    action_finish = False
                    if not __isRunning: # If the object has not moved for a period of time, start gripping.
                        continue
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(2, 500, 500)
                    AK.setPitchRangeMoving((world_X, world_Y, 12), -90, -90, 0, 1000)  # Robotic arm lifts up
                    time.sleep(1)
                    ...
                    if not __isRunning:
                        continue
                    Board.setBusServoPulse(1, servo1 - 200, 500)  # Gripper open, put down the object
                    time.sleep(0.8)
...
def run(img):
    ...
    #If an object is detected in a certain area, continue to monitor that area until there are none left.
    ...
    if not start_pick_up:
        ...
        if area_max > 2500:  # Found the largest area
            ...
            roi = getROI(box) #Get the ROI (Region of Interest) area
            get_roi = True
            ...
            distance = math.sqrt(pow(world_x - last_x, 2) + pow(world_y - last_y, 2)) #Compare with the previous coordinates to determine if there is movement
